...
export class VariationsPage implements OnInit {
  ...
  constructor(
    ...
    this.haveSize = info.size === '1' && info.variations.filter(x => x.type === 'radio').length > 0;
  ...
  radioGroupChange(event, title) {
    ...
    if (item && item.length) {
      ...
      this.cart[index].product_id = selectedItems[0].product_id;
    } else {
    ...
    */
    ...
    if (this.haveSize === false) {
      const regularItem =
      {
        ...
        product_id: this.foodInfo.product_id
      };
  ...
  addQ(index) {
    if ((this.foodInfo.cid == '13' && this.foodInfo.stock > this.sameCart[index].total) || this.foodInfo.cid != '13') {
